...
impl Index {
    ...
    pub fn writer(&self) -> Result<IndexWriter>{
        ...
        let headroom = 0.10;
        let flush_threshold_bytes = memory_budget_bytes - (memory_budget_bytes as f64 * headroom) as usize;
...
impl IndexWriter{
    ...
    pub fn flush_current_segment(&mut self ) -> Result<()> {
        if let Some(writer) = self.segment_writer.take() && writer.max_doc() > 0 {
